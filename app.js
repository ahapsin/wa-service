...
setInterval(async () => {
  const datalist = await axios
    .get("http://192.168.1.44:8080/api/send_out")
    .then(function (response) {
...
async function post(payload, id, noids) {
  const data = await axios
    .post("http://192.168.1.44:8090/send-message", payload)
    .then(function (response) {
      if (id) {
        updateFlag(id, "sent");
...
async function updateFlag(id, status) {
  ...
  const idMessage = id;
  data = await axios
    .put(`http://192.168.1.44:8080/api/send_out/${idMessage}`, {
...
server.listen(port, function () {
  console.log("apps running under port " + port + "!");
});
